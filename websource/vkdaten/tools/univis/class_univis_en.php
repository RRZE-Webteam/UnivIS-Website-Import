	...
	private function _ladeMitarbeiterAlle() {
		...
		if($this->optionen["Sortiere_Jobs"]) {
			...
			for ($i=0; $i < count($jobs); $i++) {
				...
				if(in_array($jobs[$i]["description_en"], $xjobs)) {
					continue;
				}
				if (
					(!in_array($jobs[$i]["description_en"], $jobs_vergeben))
					AND
					(count($jobs[$i]["pers"][0]["per"]) > 0)
				)
				{
					$jobs_vergeben[] = $jobs[$i]["description_en"];
				}

				for ($j=0; $j < count($jobs[$i]["pers"][0]["per"]); $j++) {
					if($personen_jobs[$jobs[$i]["pers"][0]["per"][$j]["UnivISRef"][0]["key"]]) {
						$personen_jobs[$jobs[$i]["pers"][0]["per"][$j]["UnivISRef"][0]["key"]] .= "|".$jobs[$i]["description_en"];
					}else{
						$personen_jobs[$jobs[$i]["pers"][0]["per"][$j]["UnivISRef"][0]["key"]] = $jobs[$i]["description_en"];
					}
